...
import { CoopCriteriaCreate, CoopCriteriaCreateErrors, emptyCoopCriteria } from '@/types/coop-criteria-create.model';
...
import { emptyRestriction, RestrictionCreate, RestrictionCreateErrors } from '@/types/restriction-create.model';
import {
...
import { CoopCriteriaInput } from '../CoopCriteria/CoopCriteriaInput';
import { NotesInput } from '../Notes/NotesInput';
import { RestrictionInput } from '../Restrictions/RestrictionInput';

export interface RestrictionNotesCoopCriteriaCreateProps {
  organization: OrganizationNo;
}

export function RestrictionNotesCoopCriteriaCreate({ organization }: RestrictionNotesCoopCriteriaCreateProps) {
  const [restrictionThematic, setRestrictionThematic] = useState<RestrictionCreate>(emptyRestriction('thematic'));

  const [restrictionRegional, setRestrictionRegional] = useState<RestrictionCreate>(emptyRestriction('regional'));

  const [coopCriteria, setCoopCriteria] = useState<CoopCriteriaCreate>(emptyCoopCriteria);
  ...
  const [restrictionRegionalErrors, setRestrictionRegionalErrors] = useState<RestrictionCreateErrors>({});
  ...
  useEffect(() => {
    const errorsMapped: RestrictionNotesCoopCriteriaCreateErrors = transformNestedStringifiedPropertiesToObject(errors);
    ...
    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
      <div className="border-b border-gray-200 bg-white p-6">
        <form onSubmit={handleSubmit} onKeyDown={(e) => (e.key === 'Enter' ? e.preventDefault() : void 0)} className="space-y-6">
          <CoopCriteriaInput autoFocus onChange={(data) => setCoopCriteria(data)} errors={coopCriteriaErrors} />
          <RestrictionInput type="thematic" onChange={(data) => setRestrictionThematic(data)} errors={restrictionThematicErrors} />
          <RestrictionInput type="regional" onChange={(data) => setRestrictionRegional(data)} errors={restrictionRegionalErrors} />
          <NotesInput value={notes} onChange={(data) => setNotes(data)} errors={notesErrors} />

          {/* Buttons */}
          <div className="mt-4 flex w-full justify-between">
            <SecondaryButton className="mr-4" disabled={processing} type="button" onClick={() => history.back()}>
              Zurück
            </SecondaryButton>
            <PrimaryButton
              className="ml-4"
