...
export default function TestPage() {
  ...
  function isCurrentSectionFirstSection() {
    const currentSectionId = readCurrentSectionFromDOM();
    return currentSectionId === 1;
  }

  function isCurrentSectionLastSection() {
    const currentSectionId = readCurrentSectionFromDOM();
    return currentSectionId === 3;
  }
  ...
  function scrollToNextPrev(direction: 'next' | 'previous') {
    ...
    const currentIndex = sections.findIndex((section) => sectionIdFromDomId(section.id) === currentSectionId);

    let targetIndex;
    if (direction === 'next') {
      targetIndex = currentIndex + 1 < sections.length ? currentIndex + 1 : 0; // Loop to the first section
    } else {
      targetIndex = currentIndex - 1 >= 0 ? currentIndex - 1 : sections.length - 1; // Loop to the last section
    }
  ...
  return (
    <div id="container" className="h-screen snap-y snap-mandatory overflow-y-auto">
      <div className="absolute right-5 bottom-5">
        <Button className="button m-3" onClick={() => scrollToPrev()} disabled={isCurrentSectionFirstSection()}>
          Previous
        </Button>
        <Button className="button m-3" onClick={() => scrollToNext()} disabled={isCurrentSectionLastSection() || !isCurrentSectionCleared}>
          Next
        </Button>
      </div>
