...
jest.mock('@/components/icon/check-mark', () => () => <div>CheckMark</div>);

jest.mock('@/components/icon/cross-mark', () => () => <div>CrossMark</div>);

jest.mock('@/components/invitation-codes/invitation-code-invitation-link-button', () => () => <div>InvitationCodeInvitationLinkButton</div>);

jest.mock('@/components/invitation-codes/invitation-code-add-button', () => ({
  InvitationCodeAddButton: () => <button>Add Code</button>,
}));

jest.mock('@/components/invitation-codes/invitation-code-delete-button', () => ({
  InvitationCodeDeleteButton: ({ id }: { id: number }) => <button>Delete {id}</button>,
}));

jest.mock('@/components/users/user-delete-button', () => ({
